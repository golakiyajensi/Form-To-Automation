...
export default function Sidebar({ sidebarOpen, toggleSidebar }) {
    ...
    return (
        <>
            {/* Mobile backdrop */}
            <div
                className={`fixed inset-0 bg-black/30 z-20 md:hidden transition-opacity duration-300 ${sidebarOpen ? "opacity-100" : "opacity-0 pointer-events-none"
                    }`}
            ...
            {/* Sidebar panel */}
            <aside
                className={`fixed z-30 inset-y-0 left-0 w-64 bg-[#2a3042] text-white transform transition-transform duration-300 ease-in-out ${sidebarOpen ? "translate-x-0" : "-translate-x-full"
                    }`}
            >
                ...
                <nav className="flex flex-col p-4 space-y-2">
                    {links.map(({ to, label, icon: Icon }) => (
                        <NavLink
                            ...
                            className={({ isActive }) =>
                                `flex items-center space-x-3 px-4 py-3 rounded transition font-medium ${isActive

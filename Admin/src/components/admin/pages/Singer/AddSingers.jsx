...
export default function AddSingers() {
    ...
    const EMOTIONS_LIST_ROUTE = "/admin/dashboard/singers";
    ...
    useEffect(() => {
        const fetchExistingnames = async () => {
            try {
                ...
                setExistingnames(list.map(item => item.name.toLowerCase()));
            } catch (error) {
    ...
    const validate = () => {
        ...
        if (!trimmed) validationErrors.name = "name is required";
        else if (trimmed.length < 3) validationErrors.name = "Minimum 3 characters";
        else if (trimmed.length > 100) validationErrors.name = "Maximum 100 characters";
        else if (existingnames.includes(trimmed.toLowerCase())) validationErrors.name = "name already exists";
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await axios.post(`${API_URL}/api/singer-masters`, formData, {
                headers: {
    ...
    const handleCancel = () => {
        if ((name.trim() || icon) && !confirm("Are you sure you want to discard changes?")) return;
        navigate(EMOTIONS_LIST_ROUTE);
    };
    ...
    return (
        <div className="max-w-2xl mx-auto sm:p-6">
            <div className="mb-6 flex items-center gap-3">
                ...
                <h1 className="text-2xl font-semibold text-gray-800">Add Singers</h1>
            </div>

...
export default function AddWellbeing() {
    ...
    const WELLBEING_LIST_ROUTE = "/admin/dashboard/wellbeing";
    ...
    useEffect(() => {
        const fetchExistingTitles = async () => {
            try {
                const res = await axiosInstance.get(`${API_URL}/api/wellbeing`);
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await axiosInstance.post(`${API_URL}/api/wellbeing`, {
                title: title.trim(),
                statusFlag: 1,
            });

            if (res.data.status) {
                toast.success("Wellbeing item added successfully!");
                navigate(WELLBEING_LIST_ROUTE);
            } else {
    ...
    return (
        <div className="max-w-2xl mx-auto  sm:p-6">
            <div className="mb-6 flex items-center gap-3">
                ...
                <h1 className="text-2xl font-semibold text-gray-800">Add Wellbeing Item</h1>
            </div>

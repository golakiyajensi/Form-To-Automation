...
export default function EditWellbeing() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 🔹 direct user role API call
        const res = await axiosInstance.get(`${API_URL}/api/user/${id}`);
        const data = res.data?.data;

        if (!data) {
          toast.error("User role data not found");
        ...
        setTitle(data.title || "");
        setStatusFlag(
          data.statusFlag !== undefined && data.statusFlag !== null
            ? Number(data.statusFlag)
            : 1
        );
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.status === true) {
        // <-- Corrected check
        toast.success(
          res.data.message || "Wellbeing item updated successfully"
        );
        navigate("/admin/dashboard/wellbeing");
      } else {
    ...
    return (
      <div className="p-6 max-w-4xl mx-auto">
        <div className="text-center py-8">
          ...
          <p className="mt-2">Loading wellbeing item...</p>
        </div>
      </div>
    );
  }
  ...
  if (!item) {
    return (
      <div className="p-6 max-w-4xl mx-auto">
        <div className="text-center py-8">
          <p className="text-red-500">Wellbeing item not found</p>
          <button
            onClick={handleBack}
      ...
      <div className="bg-white rounded-lg shadow-lg">
        ...
        <div className=" p-5 rounded-t-lg  border-b">
          <div className="md:flex items-center justify-between">
            ...
            <div className="text-sm opacity-90">ID: {item?.id}</div>
          </div>
        </div>

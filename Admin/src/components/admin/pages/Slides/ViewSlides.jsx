  ...
  const ViewSlides = () => {
    ...
    useEffect(() => {
      const fetchSlide = async () => {
        try {
          const response = await apiRequest(`${API_URL}/api/slide/forms/${formId}/slides`);
          if (response?.data?.length > 0) setSlide(response.data[0]);
          else toast.info("No slides available for this form.");
        } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gray-100 p-6 relative">
        ...
        <div className="mx-auto max-w-3xl bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
          ...
          <div className="p-8">
            {/* Title & Description */}
            <h1 className="text-3xl font-bold text-gray-900 mb-3">{slide.title}</h1>
            ...
            {slide.fields && slide.fields.length > 0 ? (
              <div className="space-y-6">
                {slide.fields.map((field) => (
                  ...
                  >
                    <p className="text-lg font-medium text-gray-800 mb-3">{field.label}</p>

...
export default function AddWellbeing() {
    ...
    const MENTAL_LIST_ROUTE = "/admin/dashboard/mental-health";
    ...
    const validateForm = () => {
        ...
        else if (trimmed.length < 3) newErrors.title = "Minimum 3 characters";
        else if (trimmed.length > 100) newErrors.title = "Max 100 characters";
        else if (existingTitles.includes(trimmed.toLowerCase())) newErrors.title = "Title already exists";
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await axiosInstance.post(`${API_URL}/api/mental-health`, {
                title: title.trim(),
                statusFlag: 1,
            });
    ...
    const handleCancel = () => {
        if (title.trim() && !window.confirm("Discard changes?")) return;
        navigate(MENTAL_LIST_ROUTE);
    };
    ...
    return (
        <div className="max-w-2xl mx-auto px-4 py-6">
                    <div className="flex items-center mb-6 gap-3">
                        <button
                            onClick={() => navigate(MENTAL_LIST_ROUTE)}

...
export default function EditGoals() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (!itemToEdit) {
                    toast.error("goal-masters item not found");
                    navigate('/admin/dashboard/goals');
                    return;
                }
                ...
                console.error("Error fetching goal-masters data:", error);
                toast.error("Failed to load goal-masters item");
                navigate('/admin/dashboard/goals');
            } finally {
    ...
    const res = await axiosInstance.put(
        `${API_URL}/api/goal-masters/${item.id}`,
        updateData
    );
    console.log("Update response:", res.data);

    if (res.data.status === true) { // <-- Corrected check
        toast.success(res.data.message || "goal-masters item updated successfully");
        navigate('/admin/dashboard/goals');
    } else {
        ...
        return (
            <div className="p-6 max-w-4xl mx-auto">
                <div className="text-center py-8">
                    ...
                    <p className="mt-2">Loading goal-masters item...</p>
                </div>
            </div>
        );
    }
    ...
    if (!item) {
        return (
            <div className="p-6 max-w-4xl mx-auto">
                <div className="text-center py-8">
                    <p className="text-red-500">goal-masters item not found</p>
                    <button 
                        onClick={handleBack}
                ...
                <div className=" p-5 rounded-t-lg  border-b">
                    <div className="md:flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <h1 className="text-xl md:text-2xl md:font-bold">Edit Goals</h1>
                        </div>

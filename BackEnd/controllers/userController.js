...
exports.changeUserRole = async (req, res) => {
    try {
      const user_id = req.params.user_id;
        const { role } = req.body;
        if (!user_id || !role) return res.status(400).json(response.error("user_id and role are required"));
...
exports.removeUser = async (req, res) => {
    try {
        ...
        if (!user_id) return res.status(400).json(response.error("user_id is required"));

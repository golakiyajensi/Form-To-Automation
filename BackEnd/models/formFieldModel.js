...
module.exports = {
  ...
  createFormField: async (
    ...
    const [rows] = await db.query("CALL sp_create_form_field(?,?,?,?,?,?,?)", [
      formId,
      label,
      fieldType,
      isRequired,
  ...
  getFieldsByFormId: async (formId) => {
    const [rows] = await db.query(
      "SELECT * FROM tbl_form_fields WHERE form_id = ?",
      [formId]
    );
    return rows;
  },
...
deleteFormField: async (fieldId) => {
  const [result] = await db.query("DELETE FROM tbl_form_fields WHERE field_id = ?", [fieldId]);
